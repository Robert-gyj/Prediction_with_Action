...
class MujocoEnv(gym.Env):
    ...
    def render(self, offscreen=False, camera_name="corner2", resolution=(640, 480),depth=False):
        ...
        camera_name2id = {"corner3": 3, "corner": 1, "corner2": 2, 
            "topview": 0, "gripperPOV": 5, "behindGripper": 4}
        if not offscreen:
            self._get_viewer('human').render()
        else:
            width, height = resolution
            self._get_viewer('rgb_array').render(
                    width, height, camera_id=camera_name2id[camera_name]
                )
            if not depth:
                data = self._get_viewer('rgb_array').read_pixels(
                    resolution[0], resolution[1], depth=False
                )
                return data
            else:
                data_depth = self._get_viewer('rgb_array').read_pixels(
                    resolution[0], resolution[1], depth=True
                )
                return data_depth
