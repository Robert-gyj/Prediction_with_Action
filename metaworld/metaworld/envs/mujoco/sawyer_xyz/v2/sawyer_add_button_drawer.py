...
class SawyerButtonDrawerEnvV2(SawyerXYZEnv):
    def __init__(self):
    ...
    @property
    def model_name(self):
        return full_v2_path_for('sawyer_xyz/sawyer_add_button_drawer.xml')
    ...
    def _get_pos_objects(self):
        button_pos = self.get_body_com('button') + np.array([.0, -.193, .0])
        drawer_pos = self.get_body_com('drawer_link') + np.array([.0, -.16, .0])
        if not hasattr(self, 'is_button'):
            return np.concatenate((button_pos, drawer_pos))
        pos_all = np.concatenate((button_pos, drawer_pos)) if (self.is_button) else np.concatenate((drawer_pos, button_pos))
        return pos_all

    def _get_quat_objects(self):
        return np.concatenate((self.sim.data.get_body_xquat('button'),self.sim.data.get_body_xquat('drawer_link')))
    ...
    def _get_state_rand_vec(self):
        ...
        if self._freeze_rand_vec:
            ...
            # button
            obj_low1 = np.array([-0.1, 0.85, 0.115])+np.array([0.2, 0, 0.0])*button_left
            obj_high1 = np.array([0.1, 0.9, 0.115])+np.array([0.2, 0, 0.0])*button_left
            # drawer
            obj_low2 = np.array([-0.1, 0.9, 0.0])+np.array([-0.2, 0, 0.0])*button_left
            obj_high2 = np.array([0.1, 0.9, 0.0])+np.array([-0.2, 0, 0.0])*button_left
    ...
    def reset_model(self):
        ...
        self.sim.model.body_pos[
            self.model.body_name2id('drawer')] = self.obj_init_pos2
        ...
        if self.is_button:
            ...
            self.maxDist = 0.2
            self._target_pos = self.obj_init_pos2 + np.array([.0, -.16 - self.maxDist, .09])
