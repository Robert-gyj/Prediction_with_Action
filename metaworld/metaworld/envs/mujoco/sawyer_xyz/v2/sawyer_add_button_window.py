...
class SawyerButtonWindowEnvV2(SawyerXYZEnv):
    def __init__(self):
    ...
    @property
    def model_name(self):
        return full_v2_path_for('sawyer_xyz/sawyer_add_button_window.xml')
    ...
    def _get_pos_objects(self):
        ...
        if not hasattr(self, 'is_button'):
            return np.concatenate((button_pos, window_pos))
        pos_all = np.concatenate((button_pos, window_pos)) if (self.is_button) else np.concatenate((window_pos, button_pos))
        return pos_all

    def _get_quat_objects(self):
        if not hasattr(self, 'is_button'):
            return np.concatenate((self.sim.data.get_body_xquat('button'),np.zeros(4)))
        quat_all = np.concatenate((self.sim.data.get_body_xquat('button'),np.zeros(4))) if (self.is_button) else np.concatenate((np.zeros(4),self.sim.data.get_body_xquat('window')))
        return quat_all
    ...
    def _get_state_rand_vec(self):
        ...
        if self._freeze_rand_vec:
            ...
            # button
            obj_low1 = np.array([-0.1, 0.85, 0.115])+np.array([0.25, 0, 0.0])*button_left
            obj_high1 = np.array([0.1, 0.9, 0.115])+np.array([0.25, 0, 0.0])*button_left
            # drawer
